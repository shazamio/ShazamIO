...
# Only an app `User-Agent` reaches the API. `www.shazam.com` routes a browser
#  agent away from it: a Chrome string gets `405`, a Safari one the website as
#  `200 text/html`, which the caller sees as `FailedDecodeJson` rather than as
#  a refusal. Every agent below is one the edge accepts.
